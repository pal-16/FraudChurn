...
def new_transaction():
    ...
    required = ['account_id', 'district_id', 'date', 'frequency_0', 'frequency_1', 'frequency_2', 'order_id', 'account_to', 'amount', 'bank_to', 'k_symbol']

    if not (list(data.keys()) == required):
        return 'Missing Value', 400
    
    block_index = blockchain.add_transaction(data['account_id'], data['district_id'], data['date'], data['frequency_0'], data['frequency_1'], data['frequency_2'], data['order_id'], data['account_to'], data['amount'], data['bank_to'], data['k_symbol'])
